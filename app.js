...
ONLY valid JSON. No backticks. No extra text.`;
  ...
  function printRecipe(r) {
    ...
    win.document.write(`
      ...
      <style>
        ...
        h1 { color: #b5651d; } h2 { color: #3d2b1f; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; }
        ...
        .tip { background: #fff8f0; border-left: 4px solid #b5651d; padding: 10px 14px; font-style: italic; font-size: 13px; }
  ...
  function RecipeCard({ r, showActions = true }) {
    return (
      <div style={{ background: "white", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,0.08)", marginBottom: 16 }}>
            ...
            {showActions && (
              <div style={{ display: "flex", gap: 6, marginLeft: 10 }}>
                <button onClick={() => printRecipe(r)} title="Print"
                  style={{ width: 32, height: 32, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.15)", color: "white", fontSize: 14, cursor: "pointer" }}>🖨</button>
              </div>
            )}
          </div>
      ...
      <div style={{ background: BROWN, padding: "14px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
          ...
          <div>
            ...
            <div style={{ color: "#7a6353", fontSize: 11 }}>Your complete AI-powered recipe companion</div>
          </div>
        </div>
        {/* Tabs */}
        <div style={{ display: "flex", gap: 6 }}>
          {[
            { id: "chat", label: "💬 Chef Chat" },
            { id: "cookbook", label: `📖 Cookbook (${savedRecipes.length})` },
          ].map(t => (
            <button key={t.id} onClick={() => { setTab(t.id); setSelectedRecipe(null); }}
              style={{ padding: "7px 16px", borderRadius: 20, border: "none", fontSize: 12, fontWeight: 500, cursor: "pointer", background: tab === t.id ? COPPER : "rgba(255,255,255,0.1)", color: "white", transition: "background 0.2s" }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {imageUrl && (
            <div style={{ padding: "8px 14px 0", display: "flex", alignItems: "center", gap: 10 }}>
              <img src={imageUrl} style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 10 }} alt="upload" />
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{
                  ...
                  background: msg.role === "user" ? COPPER : "white",
                  color: msg.role === "user" ? "white" : BROWN,
            ...
            {loading && (
              <div style={{ display: "flex" }}>
                <div style={{ background: "white", borderRadius: "16px 16px 16px 4px", padding: "10px 16px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", display: "flex", gap: 4 }}>
                  {[0,1,2].map(i => (
                    <div key={i} style={{ width: 7, height: 7, borderRadius: "50%", background: COPPER, animation: "bounce 1s infinite", animationDelay: `${i*0.2}s` }} />
                  ))}
                </div>
              </div>
            )}
          ...
          {selectedRecipe ? (
            <div>
              <button onClick={() => setSelectedRecipe(null)}
                style={{ marginBottom: 12, padding: "8px 16px", background: "white", border: `1.5px solid ${LIGHT}`, borderRadius: 20, fontSize: 13, color: "#7a6353", cursor: "pointer" }}>
                ← Back to cookbook
              </button>
              <RecipeCard r={selectedRecipe} />
              <div style={{ fontSize: 12, color: "#7a6353", marginBottom: 8, textAlign: "center" }}>Saved on {selectedRecipe.savedAt}</div>
            ...
            <div>
              ...
              {savedRecipes.length === 0 ? (
                ...
                <div style={{ textAlign: "center", padding: "40px 20px", color: "#7a6353", fontSize: 14 }}>
