...
function App() {
  ...
  function saveRecipeToStorage(updated) {
...
{
  "message": "Your warm friendly chat reply",
  "recipe": {
    "name": "Recipe name",
...
If modifying existing recipe, return the full updated recipe.
ONLY valid JSON. No backticks. No extra text.`;
  ...
  function printRecipe(r) {
    ...
    win.document.write(`<html><head><title>${r.name}</title>
      <style>
        ...
        h1 { color: #b5651d; } h2 { font-size: 13px; text-transform: uppercase; letter-spacing: 1px; color: #3d2b1f; }
        ...
        button { margin-top: 20px; padding: 10px 20px; background: #b5651d; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; }
        @media print { button { display: none; } }
      </style></head><body>
      <h1>${r.name}</h1><p>${r.description}</p>
      <p><span class="badge">⏱ Prep: ${r.prepTime}</span><span class="badge">🔥 Cook: ${r.cookTime}</span><span class="badge">👥 ${r.servings} servings</span><span class="badge">${r.difficulty}</span></p>
      <div class="grid">
  ...
  function RecipeCard({ r, showActions = true }) {
    return React.createElement("div", { style: { background: "white", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,0.08)", marginBottom: 12 } },
      React.createElement("div", { style: { background: BROWN, padding: "16px 18px" } },
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start" } },
          ...
          showActions && React.createElement("button", { onClick: () => printRecipe(r), title: "Print", style: { width: 32, height: 32, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.15)", color: "white", fontSize: 14, cursor: "pointer" } }, "🖨")
        ),
  ...
  return React.createElement("div", { style: { fontFamily: "sans-serif", background: CREAM, minHeight: "100vh", display: "flex", flexDirection: "column" } },
    ...
    React.createElement("div", { style: { background: BROWN, padding: "14px 20px" } },
      React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 12, marginBottom: 12 } },
        ...
        React.createElement("div", null,
          ...
          React.createElement("div", { style: { color: "#7a6353", fontSize: 11 } }, "Your complete AI-powered recipe companion")
        )
      ),
      React.createElement("div", { style: { display: "flex", gap: 6 } },
        ...[{ id: "chat", label: "💬 Chef Chat" }, { id: "cookbook", label: `📖 Cookbook (${savedRecipes.length})` }].map(t =>
          React.createElement("button", { key: t.id, onClick: () => { setTab(t.id); setSelectedRecipe(null); },
    ...
    tab === "chat" && React.createElement("div", { style: { flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" } },
      ...
      imageUrl && React.createElement("div", { style: { padding: "8px 14px 0", display: "flex", alignItems: "center", gap: 10 } },
        React.createElement("img", { src: imageUrl, style: { width: 56, height: 56, objectFit: "cover", borderRadius: 10 } }),
        React.createElement("div", { style: { fontSize: 13, color: "#7a6353" } }, "Photo ready to send"),
        React.createElement("button", { onClick: () => { setImage(null); setImageUrl(null); },
      ...
      React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 } },
        ...
        loading && React.createElement("div", { style: { display: "flex" } },
          React.createElement("div", { style: { background: "white", borderRadius: "16px 16px 16px 4px", padding: "10px 16px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", display: "flex", gap: 4 } },
            ...[0,1,2].map(i => React.createElement("div", { key: i, style: { width: 7, height: 7, borderRadius: "50%", background: COPPER, animation: "bounce 1s infinite", animationDelay: `${i*0.2}s` } }))
          )
        ),
      ...
      React.createElement("div", { style: { padding: "10px 14px", background: "white", borderTop: `1px solid ${LIGHT}`, display: "flex", gap: 8, alignItems: "center" } },
        React.createElement("button", { onClick: () => fileRef.current.click(), style: { width: 36, height: 36, borderRadius: "50%", border: `1.5px solid ${LIGHT}`, background: CREAM, fontSize: 16, cursor: "pointer", flexShrink: 0 } }, "📷"),
        React.createElement("input", { ref: fileRef, type: "file", accept: "image/*", onChange: handleImageUpload, style: { display: "none" } }),
        React.createElement("textarea", { value: input, onChange: e => setInput(e.target.value), onKeyDown: handleKey, placeholder: "Ask for a recipe, say 'make it spicier', or upload a 📷…", rows: 1, style: { flex: 1, padding: "9px 13px", borderRadius: 18, border: `1.5px solid ${LIGHT}`, fontSize: 13, fontFamily: "sans-serif", resize: "none", outline: "none", background: CREAM, color: BROWN } }),
    ...
    tab === "cookbook" && React.createElement("div", { style: { flex: 1, overflowY: "auto", padding: "16px 14px" } },
      selectedRecipe ? React.createElement("div", null,
        React.createElement("button", { onClick: () => setSelectedRecipe(null), style: { marginBottom: 12, padding: "8px 16px", background: "white", border: `1.5px solid ${LIGHT}`, borderRadius: 20, fontSize: 13, color: "#7a6353", cursor: "pointer" } }, "← Back"),
        ...
        React.createElement("button", { onClick: () => deleteRecipe(selectedRecipe.id), style: { width: "100%", padding: "11px", background: "white", border: "1.5px solid #ffaaaa", color: "#c00", borderRadius: 12, fontSize: 14, cursor: "pointer" } }, "🗑 Remove from cookbook")
      ) : React.createElement("div", null,
        React.createElement("input", { value: searchQuery, onChange: e => setSearchQuery(e.target.value), placeholder: "🔍 Search your recipes…", style: { width: "100%", padding: "11px 14px", borderRadius: 12, border: `1.5px solid ${LIGHT}`, fontSize: 14, marginBottom: 14, boxSizing: "border-box", background: "white", color: BROWN, outline: "none" } }),
          ...
          : filteredRecipes.length === 0
            ...
            : React.createElement("div", null,
                ...
                ...filteredRecipes.map(r =>
                  React.createElement("div", { key: r.id, onClick: () => setSelectedRecipe(r), style: { background: "white", borderRadius: 14, padding: "14px 16px", marginBottom: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.06)", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" } },
                    React.createElement("div", null,
                    ...
                    React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
                      React.createElement("button", { onClick: e => { e.stopPropagation(); printRecipe(r); }, style: { padding: "4px 10px", background: CREAM, border: `1px solid ${LIGHT}`, borderRadius: 8, fontSize: 12, cursor: "pointer", color: "#7a6353" } }, "🖨"),
